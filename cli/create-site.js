...
async function generateProject({
  ...
  // Generate active template configuration
  console.log(chalk.gray('  ⚙️ Generating active template configuration...'))
  await generateActiveTemplateConfig(targetDir, {
    template,
    variation,
    companyName,
    tagline,
...
async function generateActiveTemplateConfig(targetDir, config) {
  const configContent = `/**
...
export const ACTIVE_TEMPLATE_CONFIG: TemplateConfig = {
  ...
  providers: {
    ...
    ${config.paymentProvider ? `paymentProvider: '${config.paymentProvider}',` : ''}
    ${config.authProvider ? `authProvider: '${config.authProvider}',` : ''}
    ${config.cmsProvider ? `cmsProvider: '${config.cmsProvider}',` : ''}
    ${config.emailProvider ? `emailProvider: '${config.emailProvider}',` : ''}
    ${config.analyticsProvider ? `analyticsProvider: '${config.analyticsProvider}',` : ''}
  },
  metadata: {
    title: '${config.companyName} - ${config.tagline}',
    description: '${config.tagline}',
    keywords: ['${config.template}', 'website', 'professional', '${config.companyName.toLowerCase()}'],
