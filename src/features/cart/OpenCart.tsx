...
export function OpenCartButton({ quantity, ...rest }: { className?: string; quantity?: number }) {
  return (
    ...
    >
      ...
      {quantity && quantity > 0 ? (
        <span className="absolute -right-2 -top-2 flex h-[18px] min-w-[18px] items-center justify-center rounded-full border-2 border-gray-900/90 bg-orange-500 text-xs font-semibold text-white shadow-sm">
