...
export const getCSSVariable = (variable: string): string | null => {
  if (typeof window === 'undefined') return null
  return getComputedStyle(document.documentElement).getPropertyValue(variable).trim()
}

// Helper function to get breakpoint value
export const getBreakpoint = (breakpoint: keyof typeof cssVariables.breakpoints): number => {
  return cssVariables.breakpoints[breakpoint]
}

// Helper function to get color value
export const getColor = (color: keyof typeof cssVariables.colors): string => {
